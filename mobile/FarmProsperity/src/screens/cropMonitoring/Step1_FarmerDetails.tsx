...
// Wizard Step 1 — Farmer Basic Details.
...
const Step1_FarmerDetails = ({ data, onChange, onNext }: Step1Props) => {
  ...
  const {
    ...
    handleDistrictSelect, handleBlockSelect, handleVillageSelect,
  ...
  const handleVillageSmartSelect = (val: string) => {
    if (val === OTHERS_VALUE) {
      // Others selected — clear village_id, keep custom_village_name as-is
  ...
  const handleVillageCustomChange = (text: string) => {
    onChange({ custom_village_name: text });
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* District → Block cascade (standard InlinePicker — always has data) */}
        <InlinePicker
          label="District"
          required
